...
def get_merge_performance(args: argparse.Namespace, models_to_merge: list, trainers: list, logger: logging.Logger, merging_method: MergingMethod):
    ...
    # Local modification: --save_dtype. The merge is always computed in fp32
    # (models are loaded without torch_dtype), so this only controls the on-disk
    # dtype. bf16 is lossless for vLLM inference, which casts to bf16 anyway, and
    # halves the files by avoiding a redundant fp32 copy.
    if getattr(args, "save_dtype", "float32") == "bfloat16":
        logger.info("casting merged model to bfloat16 before saving")
...
# Local modification: see the note in get_merge_performance above.
parser.add_argument("--save_dtype", type=str, default="float32", choices=["float32", "bfloat16"], help="dtype to SAVE the merged model; merge compute stays fp32")
