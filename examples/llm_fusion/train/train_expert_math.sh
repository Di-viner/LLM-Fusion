#!/usr/bin/env bash
# Single-domain MATH expert, full-parameter GRPO | Qwen3-4B | FSDP | vLLM
#
# This is the template for all five per-domain experts. To train a different
# domain, change TRAIN_FILE to the matching config directory and adjust
# EXPERIMENT_NAME:
#
#   Math     ${MATH_TRAIN}      Science  ${SCIENCE_TRAIN}
#   Code     ${CODE_TRAIN}      IF       ${IF_TRAIN}
#   Agent    ${AGENT_TRAIN}
#
# The Agent domain additionally needs `rollout.agent.default_agent_loop=tool_agent`
# because its training rows are multi-turn tool-calling tasks.
#
# Validation runs the full eight-benchmark suite regardless of the training
# domain, so cross-domain transfer stays observable throughout training.
#
# Rollout dispatch (verl AgentLoopManager, per-sample via the `agent_name` column):
#   - train rows have no agent_name -> fall back to
#     rollout.agent.default_agent_loop = single_turn_agent (one turn, no tools)
#   - eval rows carry an explicit agent_name, so BFCL v3 runs as tool_agent and
#     the other seven as single_turn_agent
# The tool config is loaded either way so the BFCL eval rows can execute.
#
...
TRAIN_FILE=${TRAIN_FILE:-${MATH_TRAIN}}
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-expert_math_qwen3_4b}
SAVE_FREQ=${SAVE_FREQ:-25}
TEST_FREQ=${TEST_FREQ:-25}
TOTAL_EPOCHS=${TOTAL_EPOCHS:-10}
TOTAL_TRAINING_STEPS=${TOTAL_TRAINING_STEPS:-325}
