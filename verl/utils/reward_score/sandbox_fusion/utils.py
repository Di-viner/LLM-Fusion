...
"""
    ...
    if error_msg:
        ...
        # Log code and input only on error for brevity (truncate both; test inputs
        # for stdin problems can be thousands of lines and otherwise flood the log).
        # generation_to_log = generation[:50] + "..." if len(generation) > 50 else generation
        # stdin_to_log = stdin if stdin is None or len(stdin) <= 50 else stdin[:50] + "..."
        # logger.error(f"Case {case_index}: code: {generation_to_log}")
        # logger.error(f"Case {case_index}: input: {stdin_to_log}")
    elif api_response:
        # --- Add debug logging ---
