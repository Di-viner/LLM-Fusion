...
# Retry knobs for checkpoint shard writes (overridable via env). Networked/FUSE
# filesystems (e.g. CephFS / dop-fuse) intermittently fail large concurrent writes
# when all ranks save their shards at once, surfacing as e.g.
# "PytorchStreamWriter failed writing file data.pkl: file write failed". Retrying a
# few times (after deleting the partial file) lets a transient glitch pass instead
# of crashing the whole run.
_CKPT_SAVE_MAX_RETRIES = int(os.getenv("VERL_CKPT_SAVE_MAX_RETRIES", "5"))
_CKPT_SAVE_RETRY_DELAY = float(os.getenv("VERL_CKPT_SAVE_RETRY_DELAY", "10.0"))


def _torch_save_with_retry(obj, path, rank, what):
    """``torch.save(obj, path)`` with retries for transient shared-FS write failures.

    On each failure the partial/corrupt file is removed before retrying, and the
    delay grows linearly (delay * attempt). Re-raises the last exception if all
    attempts fail.
    """
    last_exc = None
    for attempt in range(1, _CKPT_SAVE_MAX_RETRIES + 1):
        try:
            torch.save(obj, path)
            return
        except Exception as e:  # noqa: BLE001 - retry ANY write failure, then re-raise
            last_exc = e
            try:
                ...
                pass
            log_with_rank(
                f"Saving {what} to {path} failed (attempt {attempt}/{_CKPT_SAVE_MAX_RETRIES}): {e}",
                rank=rank,
                logger=logger,
                level=logging.WARNING,
            )
            if attempt < _CKPT_SAVE_MAX_RETRIES:
                time.sleep(_CKPT_SAVE_RETRY_DELAY * attempt)
    raise RuntimeError(
        f"Saving {what} to {path} failed after {_CKPT_SAVE_MAX_RETRIES} attempts"
    ) from last_exc
        ...
        with warnings.catch_warnings():
            ...
            with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                ...
                if self.should_save_model:
                    ...
                    _torch_save_with_retry(model_state_dict, model_path, self.rank, "model")
                    log_with_rank(f"Saved model to {os.path.abspath(model_path)}", rank=self.rank, logger=logger)

                if self.should_save_optimizer:
                    optimizer_state_dict = self.optimizer.state_dict()
                    _torch_save_with_retry(optimizer_state_dict, optim_path, self.rank, "optim")
                ...
                if self.should_save_extra:
                    ...
                    _torch_save_with_retry(extra_state_dict, extra_path, self.rank, "extra_state")
