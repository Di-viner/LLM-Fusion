...
class RLHFDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        ...
        self.dataframe: datasets.Dataset = self._concatenate_dataframes(dataframes)
    ...
    @staticmethod
    def _concatenate_dataframes(dataframes: list[datasets.Dataset]) -> datasets.Dataset:
        """Concatenate datasets, tolerating minor schema mismatches across files.

        ``datasets.concatenate_datasets`` requires identical features, which breaks when mixing
        parquet files produced by different scripts/library versions (e.g. ``string`` vs
        ``large_string`` columns, or ``extra_info`` structs with different keys). When that
        happens we fall back to pyarrow's permissive promotion, which unifies compatible column
        types and fills missing struct fields with nulls.
        """
        if len(dataframes) == 1:
            return dataframes[0]
        try:
            return datasets.concatenate_datasets(dataframes)
        except ValueError as e:
            import pyarrow as pa

            logger.warning(
                "concatenate_datasets failed to align features (%s); "
                "falling back to permissive pyarrow concatenation.",
                e,
            )
            tables = [df.data.table for df in dataframes]
            merged = pa.concat_tables(tables, promote_options="permissive").replace_schema_metadata(None)
            return datasets.Dataset(merged)
    ...
    def __getitem__(self, item):
        ...
        # ``... .get(key, {})`` returns None (not the default) when the key exists
        # but holds None. That happens after concatenating parquet files with
        # divergent ``extra_info`` schemas (e.g. mixing tool-agent samples that
        # carry ``tools_kwargs`` with samples that don't -> pyarrow null-fills the
        # missing field). Coerce to {} so downstream ``tools_kwargs.get(...)`` in
        # the tool agent loop does not raise ``'NoneType' has no attribute 'get'``.
        tools_kwargs = row_dict.get("extra_info", {}).get("tools_kwargs", {}) or {}
        interaction_kwargs = row_dict.get("extra_info", {}).get("interaction_kwargs", {}) or {}
