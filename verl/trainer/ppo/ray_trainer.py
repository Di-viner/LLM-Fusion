...
class RayPPOTrainer:
    ...
    @staticmethod
    def _sanitize_data_source(data_source) -> str:
        """Turn a data_source string into a filesystem-safe file stem."""
        safe = "".join(c if (c.isalnum() or c in "._-") else "_" for c in str(data_source))
        return safe.strip("_") or "unknown"

    # Code data sources embed full test cases in ``ground_truth`` (``gts``), which can be tens to
    # hundreds of MB per prompt and get duplicated across every sampled response, exploding the
    # dump size. They are not needed to compute acc/avg@n offline, so we skip ``gts`` for them.
    _CODE_DATA_SOURCES = frozenset(
        {
            "codecontests",
            "apps",
            "codeforces",
    ...
    @staticmethod
    def _is_code_data_source(data_source) -> bool:
        """Whether a data_source is a code dataset whose ``gts`` (test cases) is huge."""
        ds = str(data_source)
        leaf = ds.rsplit("/", 1)[-1]
        if ds in RayPPOTrainer._CODE_DATA_SOURCES or leaf in RayPPOTrainer._CODE_DATA_SOURCES:
            return True
        low = leaf.lower()
        return "livecodebench" in low or "codecontests" in low
    ...
    def _write_generations(inputs, outputs, gts, scores, reward_extra_infos_dict, dump_path, global_steps,
        ...
        When ``data_sources`` is provided, each sample carries its ``data_source`` and the
        samples are split into one file per dataset under ``{dump_path}/{global_steps}/``,
        i.e. ``{dump_path}/{global_steps}/{data_source}.jsonl``. Otherwise all samples are
        written to a single ``{dump_path}/{global_steps}.jsonl`` (legacy behavior).
        """
        n = len(inputs)
        base_data = {
            "input": inputs,
        ...
        def _write_rows(file_handle, indices, omit_keys=()):
            for i in indices:
                entry = {k: v[i] for k, v in base_data.items() if k not in omit_keys}
                file_handle.write(json.dumps(entry, ensure_ascii=False, default=str) + "\n")

        if data_sources is None:
            # Legacy single-file dump.
            os.makedirs(dump_path, exist_ok=True)
            filename = os.path.join(dump_path, f"{global_steps}.jsonl")
            with open(filename, "w") as f:
                _write_rows(f, range(n))
            print(f"Dumped generations to {filename}")
            return

        # Group sample indices by data source, then write one file per dataset.
        step_dir = os.path.join(dump_path, str(global_steps))
        os.makedirs(step_dir, exist_ok=True)
        indices_by_source: dict[str, list] = defaultdict(list)
        for i, ds in enumerate(base_data["data_source"]):
            indices_by_source[str(ds)].append(i)

        for ds, indices in indices_by_source.items():
            # Skip the huge ``gts`` (test cases) for code datasets to keep dumps small.
            omit_keys = ("gts",) if RayPPOTrainer._is_code_data_source(ds) else ()
            filename = os.path.join(step_dir, f"{RayPPOTrainer._sanitize_data_source(ds)}.jsonl")
            with open(filename, "w") as f:
                _write_rows(f, indices, omit_keys=omit_keys)
            note = " (gts omitted)" if omit_keys else ""
            print(f"Dumped {len(indices)} generations for data_source={ds!r} to {filename}{note}")
    ...
    def _dump_generations(self, inputs, outputs, gts, scores, reward_extra_infos_dict, dump_path, data_sources=None):
        ...
        If ``data_sources`` is provided, samples are split into one JSONL per dataset
        (see :meth:`_write_generations`); otherwise the legacy single-file layout is used.
        """
    ...
    def _validate(self, merged: bool = False):
        ...
        # Per-batch logs are noisy and would clobber the progress bar; gate them behind a flag.
        val_verbose = self.config.trainer.get("validation_verbose", False)
        ...
        # tqdm's in-place ('\r') bar gets buried when engine/Ray logs (e.g. vLLM preemption
        # prints) stream to the same console. Periodically emit a durable, newline-terminated
        # progress line so progress stays visible/greppable amid the noise. Set the interval to
        # 0 to disable. Negative => default 60s.
        progress_interval = self.config.trainer.get("validation_progress_interval", 60)
        val_start_ts = time.time()
        last_progress_ts = val_start_ts
        for val_batch_idx, test_data in enumerate(val_pbar):
            ...
            # Durable progress line (survives interleaving with engine/Ray logs).
            if progress_interval and not val_verbose:
                done = val_batch_idx + 1
                now = time.time()
                if done == n_val_batches or (now - last_progress_ts) >= progress_interval:
                    last_progress_ts = now
                    pct = 100.0 * done / max(n_val_batches, 1)
                    print(
                        f"[validation] step {self.global_steps}: {done}/{n_val_batches} batches "
                        f"({pct:.0f}%) elapsed={now - val_start_ts:.0f}s",
                        flush=True,
                    )
        ...
        if val_data_dir:
            # Flatten per-batch data_source arrays so each dumped sample keeps its dataset
            # label and can be split into one JSONL per dataset.
            dump_data_sources = np.concatenate(data_source_lst, axis=0).tolist() if data_source_lst else None
            self._dump_generations(
                ...
                data_sources=dump_data_sources,
            )
