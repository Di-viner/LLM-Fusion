...
async def submit_request(session, semaphore, server_address, **chat_complete_request):
    # Reuse a shared session (connection pool) and cap in-flight requests with a semaphore.
    # Creating a new ClientSession per request + firing all requests at once exhausts local
    # ephemeral ports on large datasets (OSError [Errno 99] Cannot assign requested address).
...
async def generate_per_replica(
    ...
    # A single shared session reuses a bounded connection pool instead of opening one
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
        tasks = [submit_request(session, semaphore, server_address, **req) for req in chat_complete_request]
    ...
    results = await asyncio.gather(
        *[
            generate_per_replica(
                server_addresses[i], model_path, n_samples, sampling_params, chat_sub_array[i], max_concurrency
            )
...
def main(config):
    ...
    # Max in-flight requests per replica. Keeps the client from exhausting local ephemeral
    # ports / overwhelming the server when the dataset is large. Configurable via
    # `+data.max_concurrency=...`.
    max_concurrency = int(config.data.get("max_concurrency", 256))
